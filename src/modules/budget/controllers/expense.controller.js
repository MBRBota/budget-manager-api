...
router.get('/', async (req, res) => {
  try {
    ...
    const userExpenses = await sql`SELECT expense_id, expense_sum, expense_date, category_id FROM expenses INNER JOIN users USING (user_id) WHERE username=${username}`
...
router.post('/', async (req, res) => {
  try {
    ...
    await sql`INSERT INTO expenses (expense_sum, expense_date, user_id, category_id) VALUES (${expenseSum}, ${expenseDate}, ${userId}, ${categoryId})`
...
router.patch('/', async (req, res) => {
  try{
    ...
    const [userId] = await sql`SELECT user_id FROM users WHERE username=${username}`
    await sql`UPDATE expenses SET expense_sum=${expenseSum}, expense_date=${expenseDate}, category_id=${categoryId} WHERE user_id=${userId} AND expense_id=${expenseId}`
